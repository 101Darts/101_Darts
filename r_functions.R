# function check command line input
parse_input <- function(x){
	if(!has.key(arg[1],c_rx_hash)){
		message(sprintf("ARG1 %s is illegal chunk name!",arg[1]));
		quit();
		}
	if(!has.key(arg[2],c_rx_hash)){
		message(sprintf("ARG2 %s is illegal chunk name!",arg[2]));
		quit();
		}
	}


###极坐标定义二重正态分布 ，最后要多乘以一个
# 注意公共变量 u1,u2,e1,e2
fp <- function(d,r){
	r = 1/(sqrt(2*pi)*e1)*exp(-1*(r*cos(d)-u1)^2/(2*(e1^2)))*1/(sqrt(2*pi)*e2)*exp(-1*(r*sin(d)-u2)^2/(2*(e2^2)))*r	
	return(r)
}

###二维正态分布
f <- function(x,y){
	r = 1/(sqrt(2*pi)*e1)*exp(-1*(x-u1)^2/(2*(e1^2)))*1/(sqrt(2*pi)*e2)*exp(-1*(y-u2)^2/(2*(e2^2)))
	return(r)
}

###一维正态分布
fs <- function(x){
	r = 1/(sqrt(2*pi)*e1)*exp(-1*(x-u1)^2/(2*(e1^2)))
	return(r)
}


# function init c_rx_hash, c_ry_hash
init_tag_hash <- function(x){
	for(i in 1:20){
		deg=pi/2+jump_len*(i-1)
		xx=cos(deg);
		yy=sin(deg);
#		cat(sprintf("%d(%d)(%.2f), %.3f, %.3f \n",tseqs[i],i,deg,xx,yy))
#		cat(sprintf("%d(%d)\n",i,tseqs[i]));
		...
		tag<-sprintf("%dA",tseqs[i])
		c_rx_hash[tag]=azx
		c_ry_hash[tag]=azy
		c_dg_hash[tag]=deg
	
		tag=sprintf("%dB",tseqs[i])
		c_rx_hash[tag]=bzx
		c_ry_hash[tag]=bzy
		c_dg_hash[tag]=deg
	
		tag=sprintf("%dX3",tseqs[i])
		c_rx_hash[tag]=tzx
		c_ry_hash[tag]=tzy
		c_dg_hash[tag]=deg
	
		tag=sprintf("%dX2",tseqs[i])
		c_rx_hash[tag]=dzx
		c_ry_hash[tag]=dzy	
		c_dg_hash[tag]=deg
#		print(deg)
...
##
##  new_i n t e g r a l 2 . R  Double and Triple Integrals
##  fix the bug in save2list , add data valid check by yunshan.gong@outlook.com
   


new_integral2 <- function(fun, xmin, xmax, ymin, ymax, sector = FALSE,
...
#cat("__________Qsub\n");
#print(Qsub)
#cat("__________localtol\n");
#print(localtol)
#print(adjerr[1] > localtol)

if(localtol[1]){								#add by Yunshan Gong 2024/10/27, check valid     
