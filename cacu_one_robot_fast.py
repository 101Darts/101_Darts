...
if( is_number(sys.argv[1]) and is_number(sys.argv[2]) ):
	s1 = float(sys.argv[1])
	s2 = float(sys.argv[2])
	uid =sys.argv[3]					#第三个参数是uid,用以在数据库中便于查找，为每个机器人编号
else:	
	sys.stderr.write("usage : normal_distr.py 1.2 1.2 u233 \n")
	exit()


#记录开始计算时间
start_time = time.time()
rsize=100000						#bootstrap number
...
for aim in tag_list:
	...
	for (hit,pb) in result_dict.items():
		if not(hit == "MISSED"):
			...
			escore=score*pb
			if(pb>0.0001):
				print("%s\t%.1f\t%.1f\t%s\t%s\t%.3f\t%d\t%.4f"%(uid,s1,s2,aim,hit,pb,score,escore))		
				pb_missed=pb_missed-pb
	print("%s\t%.1f\t%.1f\t%s\t%s\t%.3f\t%d\t%.4f"%(uid,s1,s2,aim,"MISSED",pb_missed,0,0))		
